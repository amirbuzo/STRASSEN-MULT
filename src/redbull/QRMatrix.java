/*
 * File: QRMatrix.java Author: Jonas Lundberg Date: 23 jan 2010
 */
package redbull;
 ...
 * <p/>
 * Indexing: We follow the standard Java approach and let all indices start at 0. Hence, the index
 * of the element in the top-left corner is (0,0).
 * 
 * @author jonasl
 *
 */
public interface QRMatrix<E extends QRing<E>> {
	...
	/**
	 * Given an MxN matrix, this method returns a column (an Mx1 matrix). Exception
	...
	/**
	 * Given an MxN matrix, this method returns a row (an 1xN matrix). Exception
	 * (java.lang.IndexOutOfBoundsException) is thrown if the given column number is out of range.
	 */
	public QRMatrix<E> getRow(int row);
	
	/**
	 * Given an MxN matrix, this method returns the so-called transpose (an NxM matrix).
	 */
	public QRMatrix<E> getTranspose();
	
	/**
	 * Returns <tt>true</tt> if this matrix is a <em>square</em> matrix. Otherwise <tt>false</tt>.
	 */
	public boolean isSquare();
	
	/**
	 * Returns <tt>true</tt> if this matrix is a <em>diagonal</em> matrix. Otherwise <tt>false</tt>.
	 */
	...
	/**
	 * The matrix multiplication operation. It creates a new matrix without changing the two involved
	 * matrices. A NotCompatibleException is raised if the two matrices are not compatible.
	 */
	public QRMatrix<E> mult(QRMatrix<E> e);
	
	/**
	 * The matrix addition operation. It creates a new matrix without changing the two involved
	 * matrices. A NotCompatibleException is raised if the two matrices are not compatible.
	 */
	 ...
	 * equals()) should have identical hash values.
	 */
	public int hashCode();
